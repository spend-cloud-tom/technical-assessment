...
<template>
  <div class="border-4 border-pink-600 flex flex-col rounded-lg bg-slate-800">
    <header
      class="p-2 flex items-center bg-pink-950 border-b-4 border-pink-600"
    >
    ...
    <div class="p-2">
      <p>
        Height: <span v-if="data">{{ data.height }}</span><span v-else>...</span>
      </p>
      <p>
        Weight: <span v-if="data">{{ data.weight }}</span><span v-else>...</span>
      </p>
    </div>
    <div class="h-[23ch]">
